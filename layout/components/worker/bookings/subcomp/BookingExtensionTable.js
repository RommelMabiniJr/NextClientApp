...
const BookingExtensionTable = ({
  ...
  const handleRespondExtension = async (extensionId, response) => {
    const success = await BookingService.respondToExtensionRequest(
      booking.booking_id,
      extensionId,
      response
    );
    ...
    if (success) {
      toast.current.show({
        severity: "success",
        summary: "Success",
        detail: "Extension request accepted",
        life: 3000,
      });
  ...
  const confirmApprove = (event, extensionId) => {
    confirmPopup({
      target: event.currentTarget,
      message: "Confirm accept of this extension?",
      icon: "pi pi-info-circle",
      acceptClassName: "p-button-primary",
      rejectClassName: "p-button-secondary p-button-text",
      accept: () => handleRespondExtension(extensionId, "approved"),
    });
  };

  const confirmDecline = (event, extensionId) => {
    confirmPopup({
      target: event.currentTarget,
      message: "Confirm decline of this extension?",
      icon: "pi pi-exclamation-triangle",
      acceptClassName: "p-button-danger",
      rejectClassName: "p-button-secondary p-button-text",
      accept: () => handleRespondExtension(extensionId, "rejected"),
    });
  };
                    ...
                    {rowData.status === "pending" && (
                      <>
                        <Button
                          ...
                          icon="pi pi-check"
                          outlined
                          tooltip="Accept Extension"
                          disabled={rowData.status !== "pending"}
                          onClick={(e) =>
                            confirmApprove(e, rowData.extension_id)
                          }
                        />
                        <Button
                          size="small"
                          ...
                          disabled={rowData.status !== "pending"}
                          onClick={(e) =>
                            confirmDecline(e, rowData.extension_id)
                          }
                        />
                      </>
                    )}
