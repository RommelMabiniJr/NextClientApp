...
const DisplayHeader = ({ }) => {
    ...
    const [profileImageUrl, setProfileImageUrl] = useState('/layout/profile-default.png');
    ...
    async function getCroppedFile(imageSrc, pixelCrop, rotation = 0, flip = { horizontal: false, vertical: false }, fileName = 'cropped.jpg') {
    ...
    const footer = (
        <div>
            <Button label="Upload" onClick={handleUpload} disabled={!croppedAreaPixels} />
        ...
        <>
            <div className='text-center'>
                <div className='relative w-max mx-auto py-2'>
                    <Avatar image={profileImageUrl} alt='profile' shape='circle' className='h-8rem w-8rem md:w-10rem md:h-10rem shadow-2 cursor-pointer ' onClick={handleImgClick} />
                    <Badge value={<i className='pi pi-camera' />} size='large' className='absolute bottom-0 left-50 translate-badge cursor-pointer p-ripple' onClick={handleImgClick}></Badge>
                </div>
                <h2 className='my-2'>{session.user.firstName + ' ' + session.user.lastName}</h2>
                <h5 className='my-3 font-light'>
                    {session.user.city} | {session.user.barangay} | {session.user.street}
                </h5>
                {/* <Button label='Edit Profile' className='p-button-outlined' /> */}
                <Dialog className='w-6 relative' header='Upload Profile Image' visible={visible} onHide={() => setVisible(false)} footer={footer}>
                    <div className='flex flex-column align-items-center'>
                        ...
                        <div className='p-fluid flex w-full flex justify-content-center gap-3 mt-2'>
                            ...
                            />
                            <Button label='Remove' disabled={!previewUrl} className='p-button-danger w-4' onClick={handleCancel} />
