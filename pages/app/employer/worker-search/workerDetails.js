...
const ShowWorkerDetailsBtn = ({ worker, getDistance }) => {
    ...
    const servicesString = formatHelper.convertArrayToString(worker.services, 'service_name');

    return (
        <>
            <Button label="View Profile" className="p-button-sm p-button-secondary" onClick={() => setVisible(true)} />
            <Dialog visible={visible} className="w-full md:w-8 border-solid border-1 border-black-alpha-90" onHide={() => setVisible(false)}>
                <div className="grid">
                    <div className="col-5">
                        <center>
                            ...
                            <div className="flex flex-column align-items-center">
                                <h1 className="my-2">{worker.first_name + " " + worker.last_name}</h1>
                                ...
                                <label className="block mb-2">
                                    <i className="pi pi-map-marker" /> {getDistance(worker.city_municipality)} Kilometers - {worker.city_municipality}
                                </label>
                            </div>
                            <div className="btn-action-container">
                                <Button className="p-mt-2 p-w-13" outlined label="Hire Nanny" rounded />
                    ...
                    <div className="col">
                        <div className="flex flex-column md:flex-row flex-wrap">
                            <div style={{ flexBasis: '50%' }}>
                                ...
                                <label style={{ display: 'block', paddingBottom: '2vh' }}> {languagesString}</label>
                            </div>
                            <div style={{ flexBasis: '50%' }}>
                                <label style={{ fontWeight: 'bold' }}>
                                    <i className="pi pi-check-circle" /> Services Offered
                                </label>
                                <label style={{ display: 'block', paddingBottom: '2vh' }}> {servicesString}</label>
                            </div>
                            <div style={{ flexBasis: '50%' }}>
                                <label style={{ fontWeight: 'bold' }}>
                            ...
                            <div style={{ flexBasis: '50%' }}>
                                ...
                                <label style={{ display: 'block', paddingBottom: '2vh' }}> {worker.availability}</label>
                        ...
                        <label style={{ display: 'block', paddingBottom: '2vh' }}>{worker.bio}</label>
                    </div>
                </div>
            </Dialog>
        </>
