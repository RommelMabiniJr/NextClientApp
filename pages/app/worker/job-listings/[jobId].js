...
// THIS IS AN EXPERIMENTAL PAGE FOR VIEWING JOB POSTING DETAILS (WORK IN PROGRESS) ...
// ADDITIONAL INFO: this is an alternative to the JobDetailsDialog component

const JobPostingView = () => {
  const { data: session } = useSession();
  const router = useRouter();
  const PROVINCE = "LEYTE";
  const { jobId, elementId, tabIndex } = router.query;
      ...
      const fetchApplicationDetails = async () => {
        try {
          const response = await axios.get(
            `${serverUrl}/worker/get-post/${jobId}`
          );
    ...
    jobDetails && (
      <div className="w-full">
        ...
        <div className="grid w-full ">
          <span className="col-12 flex align-items-center mt-4">
            <Button
          ...
          <div className="grid col-8 md-8">
            <div className="col bg-white p-4 m-4 ml-6 flex flex-column justify-content-between">
              <div className="content">
                <div className="flex">
                  <div className="mr-4">
                    <Avatar
                      image={jobDetails.profile_url || defaultAvatar}
                      alt="profile"
                      shape="circle"
                  ...
                  <div className="">
                    <h5>{jobDetails.job_title}</h5>
                    {/* <p>Application Status: {jobDetails.application_status}</p> */}
                    <div className="grid w-full">
                      <div className="col flex flex-wrap py-1">
                        ...
                        <Tag
                          icon="pi pi-tag"
                          className="ml-2"
                          value={jobDetails.services[0].service_name}
                        />
                      </div>
                      <div className="col flex flex-wrap py-1">
                        <span className="text-600 font-medium ">
                      ...
                      <div className="grid w-full">
                        <div className="col mr-4 pl-3 pt-3">
                          ...
                          <div className="text-700 font-medium">
                            {dateConverter.toNumbers(jobDetails.job_start_date)}{" "}
                            - {dateConverter.toNumbers(jobDetails.job_end_date)}
                          </div>
                        </div>
                        <div className="col mr-4 pl-3 pt-3">
                          <span className="text-600 font-medium mr-1">
                          ...
                          <div className="text-700 font-medium">
                            {jobDetails.working_hours || "8:00 AM - 5:00PM"}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="col-12">
                  <div className="flex flex-wrap">
                    <div className="rate text-lg font-semibold mb-2">
                      ...
                      <span className="ml-2">{jobDetails.first_name}</span>
                    </div>
                    <span className="mx-3"> | </span>
                    <div className="rate text-lg font-semibold mb-2">
                      <span className="pi pi-map-marker"></span>
                      <span className="ml-2">
                        {jobDetails.city_municipality}
                      </span>
                    </div>
                    <span className="mx-3"> | </span>
                    <div className="rate text-lg font-semibold mb-2">
                      <span className="ml-2">{jobDetails.distance}</span>
                      <span className="sitance-value"> Kilometers</span>
                    </div>
                  </div>
                </div>
                <p className="p-2">{jobDetails.job_description}</p>
                {/* <p>
              Application Date:{" "}
