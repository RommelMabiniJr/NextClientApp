        ...
        onSubmit: async (values) => {
            try {
                ...
                const response = await axios({
                    ...
                    url: 'http://localhost:5000/worker/update-info/basics',
                });
          ...
          setTimeout(() => {
            ...
            if (!event.query.trim().length) {
              ...
              _filteredServiceOptions = services.filter((service) => {
                return service.service_name.toLowerCase().startsWith(event.query.toLowerCase());
              });
            }
              ...
              <div className="p-mt-2 flex flex-wrap">
                ...
                {formik.values.servicesOffered &&
                  formik.values.servicesOffered.map((service, index) => (
                    <Chip key={index} label={service.service_name} className="mr-1 mb-1" />
                  ))}
              </div>
            )}
          </>
    ...
    return (
        <div className='flex flex-row justify-content-between'>
            ...
            <div className="panel-fields p-mb-2 grid gap-3">
                <div className='col-6'>
                    <div className="col-fixed text-500 w-4 md:w-2 font-medium mr-4">Bio: </div>
                    {isEditMode ? renderBioField() : <div className="col text-900">{formik.values.bio}</div>}
                </div>
                ...
                <div className='col-5 grid gap-3'>
                    ...
                    <div className='col-12'>
                        ...
                        <div className='col'>
                        {isEditMode ? renderAvailabilityField() : <div className="col text-900">{formik.values.availability}</div>}
                        </div>
                    </div>
