...
import ShowPostButton from './postDetail';
...
const Posts = ({ posts }) => {
            ...
            return (
                <div className='col-12 md:col-6 lg:col-4 p-3'>
                    <Link href='/app/posts/create/'>
                        <Card title='Create a new job post' subTitle='Click this card to create a new job post.' className='h-full p-shadow-hover p-clickable transition-colors transition-duration-300 hover:surface-200'>
                            <div className='flex justify-content-center align-content-center'>
                                {/* <Image src='/layout/create-job.png' alt='Create a new job post' width='250'/> */}
                                <Button label='Create Job Post' icon='pi pi-plus' className='p-button-raised h-6rem mt-5' />
                            </div>
                        </Card>
                    </Link>
                </div>
    ...
    return (
        <div>
            <h1>My Posts</h1>
            <DataView value={updatedPosts} className='grid' itemTemplate={postTemplate} />
        </div>
    );
};
