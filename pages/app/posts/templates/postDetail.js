...
export default function ShowPostButton({ post }) {
    ...
    const footerContent = (
        <div>
            ...
            />
            <Button label="Delete" severity="danger" icon="pi pi-times" onClick={() => setDisplayDelConfirm(true)} className="p-button-danger p-button-outlined" />
            <Button label="Edit" icon="pi pi-pencil" onClick={() => Router.push({ pathname: '/app/posts/edit', query: { edit: true, post: JSON.stringify(post) } })} autoFocus />
        </div>
    );
    ...
    return (
        <>
            <Button label="View Details" icon="pi pi-external-link" onClick={() => setVisible(true)} />
            <Dialog header="Job Details" visible={visible} style={{ width: '50vw' }} onHide={() => setVisible(false)} footer={footerContent}>
                <Toast ref={toast} />
                ...
                <div className="d-flex justify-content-between">
                    <div className="grid">
                        <div className="col-6 mb-2"><strong>Start Date:</strong> {new Date(post.job_start_date).toDateString()}</div>
                        <div className="col-6 mb-2"><strong>End Date:</strong> {new Date(post.job_end_date).toDateString()}</div>
                    </div>
                    <div className="grid">
