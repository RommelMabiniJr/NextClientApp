...
export default function EmployerPosts() {
    ...
    return (
        <div className='h-screen'>
            {session ? <DisplayPosts session={session} handleSignOut={handleSignOut} /> : <div>loading...</div>}
        </div>
    )
}
...
const DisplayPosts = ({ session, handleSignOut }) => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            setIsLoading(true);
            try {
                const response = await axios.get(`http://localhost:5000/employer/post/get-posts?uuid=${session.user.uuid}`);
