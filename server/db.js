...
const insertUser = async (firstName, secondName, email, phone, hashedPassword, userType) => {
  const saltRounds = 10;
  const query = `INSERT INTO User (first_name, last_name, email, phone, password, user_type) VALUES ('${firstName}', '${secondName}',' ${email}', '${phone}', '${hashedPassword}', '${userType}')`;
  const connection = await pool.getConnection();
  try {
